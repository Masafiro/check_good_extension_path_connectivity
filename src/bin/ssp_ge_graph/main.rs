...
macro_rules! analyze_connectivity {
    ($P:expr, $FNAME:expr) => {{
        ...
        let (inv_to_id, ram_list, node_list, ge_graph, construction_time, construction_peak_bytes) =
            compute_ssp_good_extension_graph::<$P, D>();

        let (result, check_time, check_peak_bytes) =
            check_ge_path_connectivity::<$P, D>(&inv_to_id, &ram_list, &node_list, &ge_graph);
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: cargo run --release --bin ssp_ge_graph -- <p> [output_file]");
        eprintln!("Example: cargo run --release --bin ssp_ge_graph -- 11 ssp_results.csv");
        process::exit(1);
    }
